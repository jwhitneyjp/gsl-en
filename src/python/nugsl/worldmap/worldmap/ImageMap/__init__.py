...
class imageMap:
    ...
    def imagemap_anchor(self, url, coords):
        ret = "<area href='@@url@@' shape='poly' coords='@@coords@@'></area>\n"
    ...
    def imagemap_pinanchor(self, pinanchor):
        ret = "<area @@keysub@@ href='@@url@@' shape='circle' coords='@@coords@@'></area>\n"
        coords = ','.join( ['%d'%x for x in  pinanchor[1]] )
        ret = ret.replace('@@url@@', pinanchor[0] )
        # XXX keysub happens here
        ret = ret.replace('@@keysub@@', pinanchor[2])
    ...
    def imagemap_process_pinpoint(self, txt):
        ...
        # XXX Here is where we can splice in the resolved keysubs
        if self.keyfile:
            t = open(self.keyfile).read()
            keys = re.findall('@@([^@]+)@@',t)
            for key in keys:
                if self.imagemap_pinpointdata.has_key( key ):
                    t = t.replace('@@%s@@' % key, self.imagemap_pinpointdata[ key ] )
                else:
                    print 'Warning: key %s not found for %s' % (key, self.imagemap_pinpointdata['url'])
        else:
            t = ''
        self.imagemap_pinanchor_buffer.append( [self.imagemap_pinpointdata['url'], [x,y,radius], t])
        return ''
