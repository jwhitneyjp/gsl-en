        ...
        for section in self.sections():
            ...
            for option in ['latitude','longitude']:
                if not self.has_option(section,option):
                    ...
                    sys.exit()
            core_keys = []
            core_keys.append('title')
            core_keys.append('url')
            core_keys.append('name')
            core_keys.append('country')
            core_keys.append('latitude')
            core_keys.append('longitude')
            core_keys.append('pinwidth')
            for key in self.options( section ):
                if not key in core_keys:
                    m[key] = self.get(section,key)
            m['title'] = section
