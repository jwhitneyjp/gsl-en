...
// Need some cleanup of this JS clutter, here and elsewhere. Quite a grab-bag.
function purgeHrefs() {};
...
function fragHLlink() {
    if (document.getElementsByTagName) {
	    ...
	    for (i=0; i<an.length; i++) {
            if (an.item(i).getAttribute('href')) {
	            if (an.item(i).getAttribute('href').indexOf('#') >= -1) {
		            var fragment = an.item(i).getAttribute('href').substring(an.item(i).getAttribute('href').indexOf('#') + 1);
		            if (fragment && fragExclude.search(fragment)) {
...
/* Init the script ... or not. Just call with a page onload event.
